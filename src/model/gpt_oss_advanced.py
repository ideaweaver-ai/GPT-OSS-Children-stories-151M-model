...
def quantize_mxfp4(tensor: torch.Tensor, block_size: int = 32) -> MXFP4Tensor:
    ...
    block_scales = torch.max(torch.abs(tensor), dim=-1, keepdim=False)[0]
    block_scales = torch.clamp(block_scales, min=1e-8)  # Avoid division by zero
    
    # Normalize by block scales
    normalized = tensor / block_scales.unsqueeze(-1)
    ...
    scales = block_scales.to(torch.bfloat16)
...
def dequantize_mxfp4(blocks: torch.Tensor, scales: torch.Tensor, original_shape: tuple) -> torch.Tensor:
    ...
    unpacked = torch.stack([even_vals, odd_vals], dim=-1)
    unpacked = unpacked.view(unpacked.shape[:-2] + (-1,))
    
    # Expand scales to match unpacked shape
    scales_expanded = scales.unsqueeze(-1).expand_as(unpacked)
    
    # Dequantize: convert back to float and scale
    dequantized = unpacked.float() / 7.0 * scales_expanded.float()
...
class MoEExpert(nn.Module):
    ...
    def _dequantized_linear(self, x: torch.Tensor, quantized_weight: MXFP4Tensor, bias: torch.Tensor = None) -> torch.Tensor:
        ...
        # Ensure dtype compatibility - convert both to float32 for computation
        x_compute = x.float()
        weight_compute = weight.float()
        result = F.linear(x_compute, weight_compute, bias)
        ...
        if targets is not None:
            ...
            if router_logits_list and self.config.moe_aux_loss_coeff > 0:
                ...
                for i, router_logits in enumerate(router_logits_list):
                    # Get the actual layer index from MoE layers
                    if i < len(self.config.moe_layers):
                        layer_idx = self.config.moe_layers[i]
